...
async def VidWatermarkAdder(bot, cmd):
	...
	editable = await cmd.reply_text("Downloading Video ...", parse_mode="Default")
	...
	try:
		...
		logs_msg = await bot.send_message(chat_id=Config.LOG_CHANNEL, text=f"Download Started!\n\n{user_info}", reply_to_message_id=forwarded_video.message_id, disable_web_page_preview=True, parse_mode="Default", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Ban User", callback_data=f"ban_{cmd.from_user.id}")]]))
		await asyncio.sleep(5)
		c_time = time.time()
	...
	if (int(file_size) > 2097152000) and (Config.ALLOW_UPLOAD_TO_STREAMTAPE is True) and (Config.STREAMTAPE_API_USERNAME != "NoNeed") and (Config.STREAMTAPE_API_PASS != "NoNeed"):
		...
		try:
			async with aiohttp.ClientSession() as session:
				...
				await editable.edit(text_edit, parse_mode="Default", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Open Link", url=download_link)]]))
				await logs_msg.edit("Successfully Uploaded File to Streamtape!\n\nI am Free Now!", parse_mode="Default", disable_web_page_preview=True)
...
async def sts(_, m):
	...
	await m.reply_text(text=msg_text, parse_mode="Default", quote=True)
